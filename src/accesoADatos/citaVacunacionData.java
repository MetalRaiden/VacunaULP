...
//        }
//    }
        //otra version que solo modifica la fecha de la cita
        String sql = "UPDATE citaVacunacion SET fechaHoraCita = ? WHERE idCita = ?";

        try (PreparedStatement preparedStatement = conexion.prepareStatement(sql)) {
            preparedStatement.setString(1, cita.getFechaHoraCita());
            preparedStatement.setInt(2, cita.getCodCita());  // Supongo que tienes un campo "idCita" para identificar la cita
            int exito = preparedStatement.executeUpdate();

            if (exito == 1) {
                JOptionPane.showMessageDialog(null, "Cita de vacunación modificada con éxito.");
            } else {
                JOptionPane.showMessageDialog(null, "No se pudo modificar la cita de vacunación.");
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error al modificar la cita de vacunación: " + e.getMessage());
        }
...
//    }
    ...
    public void VacunaAplicada(int codCita) {
        ...
        try (PreparedStatement preparedStatement = conexion.prepareStatement(sql)) {
            ...
            if (exito == 1) {
                JOptionPane.showMessageDialog(null, "La vacuna ha sido aplicada con éxito.");
            } else {
                JOptionPane.showMessageDialog(null, "No se pudo establecer la fecha y hora de la aplicación de la vacuna.");
            }
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error al actualizar la fecha y hora de la aplicación de la vacuna: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (fechaHoraColocaString != null) {
                    try {
                        LocalDateTime fechaHoraColoca = LocalDateTime.parse(fechaHoraColocaString, DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                        CitaVacunacion.setFechaHoraVacunacion(fechaHoraColoca);
                    } catch (DateTimeParseException e) {
                        // Manejar el error de análisis de fecha si es necesario
                        e.printStackTrace();
                    }
                }
